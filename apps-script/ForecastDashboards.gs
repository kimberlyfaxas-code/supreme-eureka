...
sh.getRange("D6").setFormula(`=IFERROR(SUM(FILTER(${BA}, ${maskB})),0)`).setNumberFormat("$#,##0;($#,##0)");
  ...
  // Monthly table
  const startRow = 11;
  sh.getRange(startRow, 1, 1, 5).setValues([["Month","Forecast","Baseline","Variance","Retention %"]])
  ...
  for (let i = 0; i < months.length; i++) {
    ...
    const mCell = `A${r}`;
    // Force boolean for FILTER
const monthMaskForecast = `((${mask})*N(${FM}=${mCell}))>0`;
const monthMaskBaseline = `((${mask})*N(${BM}=${mCell}))>0`;
    sh.getRange(r, 2).setFormula(`=IFERROR(SUM(FILTER(${FA}, ${monthMaskForecast})),0)`).setNumberFormat("$#,##0;($#,##0)");
    sh.getRange(r, 3).setFormula(`=IFERROR(SUM(FILTER(${BA}, ${monthMaskBaseline})),0)`).setNumberFormat("$#,##0;($#,##0)");
    sh.getRange(r, 4).setFormula(`=B${r}-C${r}`).setNumberFormat("$#,##0;($#,##0)");
    sh.getRange(r, 5).setFormula(`=IF(C${r}=0,0,B${r}/C${r})`).setNumberFormat("0.0%");
  }
  // Quarterly table
  const qRow = 25;
  sh.getRange(qRow, 1, 1, 5).setValues([["Quarter","Forecast","Baseline","Variance","Retention %"]])
    .setFontWeight("bold").setBackground("#eef2f7");
  const quarters = [
...
const qMaskForecast = `((${mask})*N(ISNUMBER(MATCH(${FM}, ${dateConsts}, 0))))>0`;
const qMaskBaseline = `((${mask})*N(ISNUMBER(MATCH(${BM}, ${dateConsts}, 0))))>0`;
    sh.getRange(r, 2).setFormula(`=IFERROR(SUM(FILTER(${FA}, ${qMaskForecast})),0)`).setNumberFormat("$#,##0;($#,##0)");
    sh.getRange(r, 3).setFormula(`=IFERROR(SUM(FILTER(${BA}, ${qMaskBaseline})),0)`).setNumberFormat("$#,##0;($#,##0)");
    sh.getRange(r, 4).setFormula(`=B${r}-C${r}`).setNumberFormat("$#,##0;($#,##0)");
    sh.getRange(r, 5).setFormula(`=IF(C${r}=0,0,B${r}/C${r})`).setNumberFormat("0.0%");
  }
  SpreadsheetApp.flush();
  // Line chart (Month, Forecast, Baseline)
  FD_upsertLineChart_(
    sh,
